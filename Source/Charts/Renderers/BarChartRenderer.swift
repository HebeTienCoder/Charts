...
open class BarChartRenderer: BarLineScatterCandleBubbleRenderer
{
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: IBarChartDataSet, index: Int)
    {
        ...
        if stackSize > 1 {
            ...
            for firstIndexInBar in stride(from: 0, to: buffer.rects.count, by: stackSize)
            {
                ...
                if dataSet.roundedCorners.contains(.topLeft) || dataSet.roundedCorners.contains(.topRight)
                {
                    topIndexInBar = findTopIndexInBar(barRects: buffer.rects,
                                                      firstIndexInBar: firstIndexInBar,
                                                      lastIndexInBar: lastIndexInBar)
                }
                if dataSet.roundedCorners.contains(.bottomLeft) || dataSet.roundedCorners.contains(.bottomRight)
                {
                    bottomIndexInBar = findBottomIndexInBar(barRects: buffer.rects,
                                                            firstIndexInBar: firstIndexInBar,
                                                            lastIndexInBar: lastIndexInBar)
                }
    ...
    internal func createBarPath(for rect: CGRect, roundedCorners: UIRectCorner, cornerRadius: CGFloat) -> UIBezierPath {
        let modifyCornerRadius = min(cornerRadius, rect.size.width/2)
